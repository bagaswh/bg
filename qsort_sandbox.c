...
void print_arr_state(int *arr, ssize_t i, ssize_t j, ssize_t p, ssize_t r,
                     ssize_t pivot) {
  ...
  for (size_t ii = 0; ii < r; ii++) {
    ...
    if (has_arrow_ptr) {
      if (has_arrow_ptr_for_i) printf(COLOR_BLUE "i" COLOR_RESET);
      if (has_arrow_ptr_for_j) printf(COLOR_GREEN "j" COLOR_RESET);
      if (has_arrow_ptr_for_p) printf(COLOR_RED "p" COLOR_RESET);
      if (has_arrow_ptr_for_pivot) printf(COLOR_PURPLE "r" COLOR_RESET);
      printf(" ");
    } else {
      int digsc = digits_count(arr[ii]);
      printf(" ");
...
void partition(int *items, size_t p, size_t r) {
  int *pivot = &items[r - 1];
  ssize_t i = p - 1;
  ssize_t j = p;
  while (j < r - 2) {
    ...
    if (items[j] <= *pivot) {
      i++;
      printf(
          "%d at index %ld is lt pivot (%d); swapping this with %d at index "
          "%ld\n",
...
void hoare_partition(int *arr, size_t p, size_t r) {
  int *pivot = &arr[p];
  ssize_t i = p;
  ssize_t j = r;
  while (1) {
...
struct rand init_rand() {
  struct timespec ts = {0, 0};
  clock_gettime(CLOCK_MONOTONIC, &ts);
  return (struct rand){.seed = ts.tv_nsec};
}

unsigned fast_rand(struct rand *r) {
  r->seed = 214013 * r->seed + 2531011;
  return (r->seed >> 16) & 0x7fff;
}

unsigned fast_rand_n(struct rand *r, int n) { return fast_rand(r) % n; }

size_t sort_insertion(int *arr, size_t n) {
...
int mo3_pick_median(int *arr, int *pivot_picker_buf, struct rand *rng,
                    size_t n) {
  ...
  printf("0=%d 1=%d 2=%d\n", pivot_picker_buf[0], pivot_picker_buf[1],
         pivot_picker_buf[2]);
  if (arr[one_idx] == pivot_picker_buf[1]) return one_idx;
  if (arr[two_idx] == pivot_picker_buf[1]) return two_idx;
  if (arr[three_idx] == pivot_picker_buf[1]) return three_idx;
}

int mo3_partition(int *arr, int *pivot_picker_buf, struct rand *rng, size_t p,
                  size_t r) {
...
void bg_qsort(int *arr, int *pivot_picker_buf, struct rand *rng, size_t p,
              size_t r) {
  if (r - p <= 10) {
...
int main(void) {
  ...
  bg_qsort(arr, pivot_picker_buf, &rng, 0, arrlen(arr, int));
