...
Stack *stack_create_with_allocator(Allocator *allocator, size_t elem_size) {
  return slice_new_with_allocator(allocator, 1, 0, elem_size);
}

Stack *stack_create(size_t elem_size) { return slice_new(1, 0, elem_size); }
...
Stack *stack_push(Stack *s, void *item) { return slice_append(s, item); }
...
bool stack_empty(Stack *s) { return slice_get_len(s) <= 0; }
