...
struct bg_slice *bg_slice_new_with_allocator(Allocator *allocator, size_t cap,
...
struct bg_slice *bg_slice_copy_from_ptr_with_allocator(Allocator *allocator,
                                                       void *ptr, size_t cap,
                                                       size_t len,
                                                       size_t elem_size) {
  if (cap == 0 || elem_size == 0) {
...
struct bg_slice *bg_slice_new_from_ptr_with_allocator(Allocator *allocator,
                                                      void *ptr, size_t cap,
                                                      size_t len,
                                                      size_t elem_size) {
  if (cap == 0 || elem_size == 0) {
...
struct bg_slice *bg_slice_char_new_with_allocator(Allocator *allocator,
                                                  size_t cap, size_t len) {
  return bg_slice_new_with_allocator(allocator, cap, len, sizeof(char));
}

struct bg_slice *bg_slice_char_new_from_str_with_allocator(Allocator *allocator,
                                                           const char *str) {
  return bg_slice_copy_from_ptr_with_allocator(allocator, str, strlen(str) + 1,
 ...
 *
 * TODO:
 * This has a potentially critical issue. When the newly created slice needs to
 * grow, the original slice s->data might now be some random memory blocks! So I
 * think either we copy the data; or during grow, we allocate new memory using
 * malloc and let the original memory untouched. Performing malloc on grow has
 * major drawback: we'll miss optimization from realloc which potentially just
 * need to coalesce with adjacent blocks. If using malloc instead, there will be
 * those expensive block finding and stuff (though it's implementation
 * dependent).
 *
 */
struct bg_slice *bg_slice_new_from_bg_slice_with_allocator(Allocator *allocator,
                                                           struct bg_slice *src,
                                                           size_t start,
                                                           ssize_t len) {
 ...
 */
struct bg_slice *bg_slice_copy_from_bg_slice_with_allocator(
    Allocator *allocator, struct bg_slice *src, size_t start, ssize_t len,
    ssize_t cap) {
  if (start >= src->cap || len > src->cap) {
    return NULL;
  }
...
struct bg_slice *bg_slice_new_from_slice(struct bg_slice *src, size_t start,
                                         ssize_t len) {
  return bg_slice_new_from_bg_slice_with_allocator(&Malloc_Allocator, src,
                                                   start, len);
}

struct bg_slice *bg_slice_copy_from_slice(Allocator *allocator,
...
struct bg_slice *bg_slice_append_n(struct bg_slice *s, void *const item,
                                   size_t n) {
  if ((s->len + n) >= s->cap) {
...
void bg_slice_range(struct bg_slice *s, void *ctx,
...
size_t bg_slice_sort_insertion(struct bg_slice *s, void *ctx,
...
size_t bg_slice_sort(struct bg_slice *s, void *ctx, bg_slice_sort_key_fn key_fn,
                     bg_slice_sort_comparator comparator) {
